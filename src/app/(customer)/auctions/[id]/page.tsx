...
const toPics = (s?: string) =>
  (s || '')
    ...
    .map(p => p.trim())
    .filter(Boolean)
    .map(p => (/^https?:\/\//.test(p) ? p : `${API}${p.startsWith('/') ? '' : '/'}${p}`));
...
export default function AuctionDetailPage() {
  ...
  // min increment + ปิดรอบ
  const minInc = Math.max(1, data?.min_increment ?? 1);
  const closedByTime = data ? new Date(data.end_time).getTime() <= Date.now() : false;
  ...
  useEffect(() => {
    ...
    const load = async () => {
      try {
        const res = await fetch(`${API}/auction/${id}`, { credentials: 'include' });
        if (!res.ok) throw new Error(`HTTP ${res.status}`);
        const json: Auction = await res.json();
        setData(json);
        setErr('');
        // ตั้งค่าเริ่ม
        setAmount(prev => (prev === '' ? json.current_price + Math.max(1, json.min_increment ?? 1) : prev));
  ...
  // นับถอยหลังแบบง่าย (อัปเดตด้วย polling)
  const left = useMemo(() => {
    if (!data) return '';
    const ms = Math.max(0, new Date(data.end_time).getTime() - Date.now());
    const s = Math.floor(ms / 1000);
    const h = Math.floor(s / 3600);
    const m = Math.floor((s % 3600) / 60);
    const ss = s % 60;
    return `${String(h).padStart(2, '0')}:${String(m).padStart(2, '0')}:${String(ss).padStart(2, '0')}`;
  }, [data?.end_time, data?.current_price]);

  // ส่งบิด
  const submitBid = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!data || amount === '') return;
    const min = data.current_price + minInc;
    if (amount < min) {
    ...
    try {
      ...
      if (!res.ok || json?.ok === false) throw new Error(json?.message || 'บิดไม่สำเร็จ');

      // อัปเดตทันที
      const newPrice = json.current_price ?? Number(amount);
      setData({ ...data, current_price: newPrice });
      setAmount(newPrice + minInc);
    } catch (e:any) {
  ...
  if (!data) return <main className="pt-36 px-6 text-red-600">{err || 'ไม่พบรอบนี้'}</main>;

  // map เป็น product โครงหน้าปกติของเค้า
  const product = {
    Pname: data.PROname,
    Ppicture: data.PROpicture,
      Pdetail: data.PROdetail ?? '',
    Pprice: data.current_price,
    Pid: data.PROid,
    Pstatus: data.status,
  };
      ...
      <div className="flex flex-col lg:flex-row gap-10 items-start">
        ...
        <div className="w-full lg:w-1/2">
          ...
          <div className="w-full aspect-[4/3] rounded-lg overflow-hidden shadow">
            {mainImgFull ? (
              <img src={mainImgFull} alt={product.Pname} className="w-full h-full object-cover" />
            ) : (
              <div className="w-full h-full bg-gray-100 flex items-center justify-center text-gray-400">ไม่มีรูป</div>
            )}
          </div>
        ...
        <div className="w-full lg:w-1/2 flex flex-col gap-6">
          ...
          <div className="space-y-3">
            ...
            <h2 className="text-[28px] font-extrabold text-black">
              ประมูลตอนนี้ที่{' '}
              <span className="text-red-600 text-[36px] font-extrabold">{baht(product.Pprice)}</span>
            </h2>

            <div className="text-sm text-gray-700">
              เวลาคงเหลือ: <span className={`font-mono ${closed ? 'text-red-600' : ''}`}>{left}</span>
            </div>

            <form onSubmit={submitBid} className="space-y-2">
              <div className="text-sm text-gray-700">
                ขั้นต่ำต้องมากกว่า{' '}
                {new Intl.NumberFormat('th-TH').format(data.current_price + minInc)} บาท
              </div>
              <div className="flex gap-2">
                <input
  type="number"
  min={data.current_price + minInc}
  step={minInc}
  required
  value={amount}
...
/>
            ...
            <p className="text-xs text-gray-700">รหัสสินค้า: cac:{String(product.Pid).padStart(4, '0')}</p>
