      ...
      <div className="w-full max-w-3xl mx-auto p-6">
        ...
        <form onSubmit={handleSubmit} className="bg-white rounded-2xl shadow-lg border-2 border-gray-200 p-8 space-y-6">
          ...
          <div>
            ...
            {form.Ppicture && (
              <div className="flex flex-wrap gap-3 mt-4 max-h-64 overflow-auto p-4 bg-gray-50 rounded-xl">
                {form.Ppicture.split(',').map((path, idx) => (
                  <div key={idx} className="relative w-32 h-32 group">
                    ...
                    <button
                      ...
                      className="absolute -top-2 -right-2 bg-red-500 hover:bg-red-600 text-white rounded-full w-7 h-7 flex items-center justify-center shadow-lg transition-all duration-200"
                    >
                      ×
                    </button>
                  </div>
          ...
          {/* สถานะสินค้า */}
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">สถานะสินค้า</label>
            <select
              name="Pstatus"
              value={form.Pstatus}
              onChange={handleChange}
